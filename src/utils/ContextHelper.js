...
export default class ContextHelper {
  ...
  parseRequest(req) {
    ...
    if (headers) {
      ...
      this.context.device.desktop =
        headers.is_desktop && headers.is_desktop.toLowerCase() === 'true';
      this.context.device.tablet = headers.is_tablet && headers.is_tablet.toLowerCase() === 'true';
      this.context.route.link_prefix =
        headers['path.prefix'] && headers['path.prefix'].toLowerCase();
    }
  }
  ...
  parseConfig(config) {
    ...
    this.context.device.mobile =
      this.context.device.mobile || (IS_MOBILE && IS_MOBILE.toLowerCase() === 'true');
    this.context.device.desktop =
      this.context.device.desktop || (IS_DESKTOP && IS_DESKTOP.toLowerCase() === 'true');
    this.context.device.tablet =
      this.context.device.tablet || (IS_TABLET && IS_TABLET.toLowerCase() === 'true');
